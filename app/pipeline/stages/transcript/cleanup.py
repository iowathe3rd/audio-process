...
from app.domain.contracts import TranscriptCleaner
...
class SegmentCleaner(TranscriptCleaner):
    """Cleans transcript segments by removing duplicates and short segments."""

    def __init__(self, min_duration_ms: int, duplicate_window_ms: int) -> None:
    ...
    def cleanup(
        ...
        duplicate_window_ms: int | None = None,
    ) -> tuple[list[PipelineSegment], dict[str, Any]]:
        effective_min_ms = min_duration_ms if min_duration_ms is not None else self._min_duration_ms
        effective_dup_ms = duplicate_window_ms if duplicate_window_ms is not None else self._duplicate_window_ms

        min_duration_sec = max(0.0, float(effective_min_ms) / 1000.0)
        duplicate_window_sec = max(0.0, float(effective_dup_ms) / 1000.0)
        ...
        report: dict[str, int | float] = {
            ...
            "cleanup_min_duration_ms": int(effective_min_ms),
            "cleanup_duplicate_window_ms": int(effective_dup_ms),
        }
